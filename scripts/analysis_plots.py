        ...
        for metric in METRICS:
            for (o1, o2) in optim_pairs:
                ...
                try:
                    ...
                    xs = np.array(range(21))/20.
                    plot = MultiPlot(xs=xs, title=title, colors=COLORS)
                    for L in BLOCK_LAYERS:
                        plot.add_line((L,ys[L]), marker='o')
                    plot.create_plot(x_label="Fraction of Total Epochs", y_label="Layer Similarity")
                    plot.save_fig(output_dir=PLOT_DIR, filename=filename, transparent=False)
        ...
        for metric in METRICS:
            for optim in OPTIMIZERS:
                ...
                for L in BLOCK_LAYERS:
                    plot.add_line((L,ys[L]), marker='o')
                plot.create_plot(x_label="Fraction of Total Epochs", y_label="Layer Similarity")
                plot.save_fig(output_dir=PLOT_DIR, filename=filename, transparent=True)
                logger.info(f"plot {cfg.which_plot} saved at: {PLOT_DIR}/{filename}")
                ...
                plot = MultiPlot(xs=xs, title=title, colors=COLORS, cosine_overlay=True)
                for L in BLOCK_LAYERS:
                    ys_diff[L] = [0]
                    for i in range(19):
                ...
                for L in BLOCK_LAYERS:
                    ...
                    plot.add_line((L,ys_diff[L]), marker='o', linestyle='-')
                
                ys_diff_total = ys_diff_total * 0.5 / (np.sum(ys_diff_total)/20.)
                plot.add_line(("Net Delta", ys_diff_total), marker='o')

                plot.create_plot(x_label="Fraction of Total Epochs", y_label="Change in Layer Similarity")
                plot.save_fig(output_dir=PLOT_DIR, filename=filename, transparent=False)
        ...
        for agg in AGG_METRICS.keys():
            ...
            for metric in METRICS:
                ...
                for (o1, o2) in optim_combs:
                    ...
                    title = f"{AGG_METRICS[agg]} {metric.upper()} Similarity\nbetween CAEs Trained Using {o1}, {o2} Optimizers"
                ...
                for (o1, o2) in ys.keys():
                    label = f"Sim({o1}, {o2})"
                    plot.add_line((label,ys[(o1, o2)]), marker='o')
        ...
        * clustering?
        ...
        for method in VIS_METHODS.keys():
            for agg in AGG_METRICS.keys():
                ...
                filename = f"plot_4_{agg}_{method}.png"
                ...
                plt.title(title)
                for n, l in enumerate(plot_dict.keys()):
                    c = [COLORS[n] for i in range(len(plot_dict[l][0]))]
                    plt.scatter(plot_dict[l][0], plot_dict[l][1], label=l, c=c)
                plt.legend()
                
                dir = Path(PLOT_DIR)
                dir.mkdir(parents=True, exist_ok=True)
                dir = dir / filename
                plt.savefig(dir, bbox_inches='tight', transparent=False)
