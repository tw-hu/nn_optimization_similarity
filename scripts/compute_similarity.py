...
def main(cfg: DictConfig):
    ...
    plot_dir, sim_dir = output_dir / "plots", output_dir / "similarities"
    ...
    if cfg.state == "final":
        ...
        similarity.plot_similarities(
            title=f"Layer similarity at model state final",
        ...
        for n, s in enumerate(states_a):
            ...
            similarity.compute_similarities(metric=cfg.sim_metric) 
            similarity.plot_similarities(
                title=f"Layer similarity at model state epoch_fraction {n}/20",
                output_folder="outputs/plots",
                transparent=True
            )
            similarity.write_similarities(output_folder="outputs/similarities")
        ...
        for n, s in enumerate(states_b):
            ...
            path_b = Path(f"experiments/{file_b}/activations/{s}.pt")
            actv_b = torch.load(path_b, map_location=device)

            similarity = SimilarityComputer(actv_b, actv_a, experiment_name=name, model_state=f"{n}_v_final")
            similarity.compute_similarities(metric=cfg.sim_metric)
            similarity.plot_similarities(
                title=f"{cfg.sim_metric} similarity between state {s} and final state",
                output_folder="outputs/plots",
                transparent=True
            )
            similarity.write_similarities(output_folder="outputs/similarities")
